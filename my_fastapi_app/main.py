    ...
    def __extract_ouput(self, 
        ...
        for i in range(rows):
            ...
            if (classes_score[class_id] > score):
                ...
                box = [left, top, width, height]  # Use list instead of numpy array
                boxes.append(box)
        ...
        # Handle the case when NMSBoxes returns empty result
        if len(indexes) > 0:
            for i in indexes:
                r_class_ids.append(class_ids[i])
                # Convert numpy float32 to Python float and multiply by 100
                r_confs.append(float(confs[i]) * 100)
                r_boxes.append(boxes[i])  # boxes[i] is already a list
